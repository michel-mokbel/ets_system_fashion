...
<body>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-check form-switch mb-3">
                                    <input class="form-check-input" type="checkbox" id="weeklyPricingToggle" name="weekly_pricing_enabled" value="1">
                                    <label class="form-check-label" for="weeklyPricingToggle">Enable weekly pricing for a single store</label>
                                </div>
                            </div>
                            <div class="col-md-6 mb-3" id="weeklyStoreContainer" style="display:none;">
                                <label class="form-label">Store (required)</label>
                                <select class="form-select" name="weekly_store_id" id="weeklyStoreSelect">
                            ...
                            <div class="col-md-12" id="weeklyPricesContainer" style="display:none;">
                                <div class="row g-2">
                                    <?php
                                    $days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
                                    for ($d = 0; $d < 7; $d++) {
                                        echo '<div class="col-6 col-md-3 col-lg-2">'
                                            . '<label class="form-label">' . $days[$d] . '</label>'
                                            . '<div class="input-group input-group-sm">'
                                            . '<span class="input-group-text">CFA</span>'
                                            . '<input type="number" step="0.01" min="0" class="form-control" name="weekly_price_' . $d . '" placeholder="0.00">'
                                            . '</div>'
                                            . '</div>';
                                    }
                                    ?>
                                </div>
                                <small class="text-muted">When enabled, this item will be assigned only to the selected store and POS will use the day-specific price.</small>
                            </div>
                        </div>
                        <div class="row">
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                const toggle = document.getElementById('weeklyPricingToggle');
                                const storeC = document.getElementById('weeklyStoreContainer');
                                const pricesC = document.getElementById('weeklyPricesContainer');
                                const assignments = document.querySelectorAll("input[name='store_assignments[]']");

                                function updateWeeklyUI() {
                                    const on = toggle && toggle.checked;
                                    if (storeC) storeC.style.display = on ? '' : 'none';
                                    if (pricesC) pricesC.style.display = on ? '' : 'none';
                                    assignments.forEach(i => {
                                        i.disabled = on;
                                        if (on) i.checked = false;
                                    });
                                }
                                if (toggle) toggle.addEventListener('change', updateWeeklyUI);
                                updateWeeklyUI();
                            });
                        </script>
                        <div class="mb-3">
                ...
                <form action="../ajax/process_inventory.php" method="POST" class="needs-validation" data-reload-table="#inventoryTable" novalidate enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-check form-switch mb-3">
                                    <input class="form-check-input" type="checkbox" id="editWeeklyPricingToggle" name="weekly_pricing_enabled" value="1">
                                    <label class="form-check-label" for="editWeeklyPricingToggle">Enable weekly pricing for a single store</label>
                                </div>
                            </div>
                            <div class="col-md-6 mb-3" id="editWeeklyStoreContainer" style="display:none;">
                                <label class="form-label">Store (required)</label>
                                <select class="form-select" name="weekly_store_id" id="editWeeklyStoreSelect">
                                    <option value="">Select store</option>
                                    <?php
                                    $stores_query = "SELECT * FROM stores ORDER BY name";
                                    $stores_result = $conn->query($stores_query);
                            ...
                            <div class="col-md-12" id="editWeeklyPricesContainer" style="display:none;">
                                <div class="row g-2">
                                    <?php
                                    $days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
                                    for ($d = 0; $d < 7; $d++) {
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                const toggle = document.getElementById('editWeeklyPricingToggle');
                                const storeC = document.getElementById('editWeeklyStoreContainer');
                                const pricesC = document.getElementById('editWeeklyPricesContainer');
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="row mb-2" id="detailWeeklyPricingRow" style="display:none;">
                                <div class="col-md-4 text-muted">Weekly Pricing:</div>
                                <div class="col-md-8" id="detailWeeklyPricing"></div>
                            </div>
                            <div class="row mb-2">
    ...
    <script>
    window.canEditInventory = <?php echo (is_admin() || is_inventory_manager()) ? 'true' : 'false'; ?>;
    </script>
